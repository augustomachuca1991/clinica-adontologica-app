    ...
    <header className={`fixed top-0 right-0 h-16 bg-card border-b border-border shadow-clinical-sm z-30 transition-all duration-slow ${sidebarCollapsed ? "left-20" : "left-0 lg:left-64"}`}>
      <div className="h-full flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => console.log("Quick add clicked")} className="relative" iconName="Plus" aria-label="Quick add" />
            ...
            {showNotifications && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-card border border-border rounded-lg shadow-clinical-lg overflow-hidden">
                <div className="px-4 py-3 border-b border-border bg-muted/50">
                  <h3 className="font-headline font-semibold text-sm">Notifications</h3>
                </div>
                ...
                <div className="px-4 py-3 border-t border-border bg-muted/50">
                  <button className="text-sm text-primary hover:text-primary/80 font-medium transition-colors duration-base">View all notifications</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            {showUserMenu && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-card border border-border rounded-lg shadow-clinical-lg overflow-hidden">
                <div className="px-4 py-3 border-b border-border bg-muted/50">
                  <div className="font-medium text-sm text-foreground">Dr. Sarah Johnson</div>
                  <div className="text-xs text-muted-foreground">sarah.johnson@dentalcare.com</div>
                </div>
                <div className="py-2">

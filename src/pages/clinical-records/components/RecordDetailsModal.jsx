    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-lg shadow-clinical-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                      {t("records.recordsModal.tabs.overview.treatmentDate")}
                    </label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                      {t("records.recordsModal.tabs.overview.toothNumber")}
                    </label>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                      {t("records.recordsModal.tabs.overview.treatmentType")}
                    </label>
                    <p className="text-sm text-foreground mt-1 capitalize">
                      {currentRecordData?.treatmentName || t("common.notAvailable")}
                    </p>
                  </div>
                  ...
                  {currentRecordData?.duration && (
                    <div>
                      <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                        {t("records.recordsModal.tabs.overview.duration")}
                      </label>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-foreground">
                        {currentRecordData.clinical_notes.find((n) => n.type === "followUp").content}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === "images" && (
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  {isUploading
                    ? t("records.recordsModal.tabs.images.button.uploading")
              ...
              {currentAttachments && currentAttachments.length > 0 ? (
                ...
                <div
                  className="text-center py-12 hover:bg-muted cursor-pointer rounded-lg border-border border-2 border-dashed"
                  onClick={() => fileInputRef.current.click()}
                >
          ...
          {activeTab === "history" && (
            <div className="space-y-4">
              <h4 className="text-base font-headline font-semibold text-foreground mb-4">
                {t("records.recordsModal.tabs.history.treatmentHistory")}
              </h4>
                  ...
                  {history?.length > 0 ? (
                    history.map((item) => (
                      <div key={item.id} className="relative pl-12 pb-6">
                        <div
                          className={`absolute left-2.5 top-2 w-3 h-3 rounded-full bg-primary border-2 border-card ${item.id === record.id ? "bg-primary scale-125" : "bg-muted-foreground/40"}`}
                        />
                        <div className="bg-muted border border-border rounded-lg p-4">
                          ...
                          <span className="text-sm text-muted-foreground">
                            {t("records.recordsModal.tabs.history.activity.by") || "por"}
                          </span>{" "}
                          {/* Nombre del doctor resaltado */}
                    ...
                    <div className="pl-12 py-4 text-sm text-muted-foreground italic">
                      {t("records.recordsModal.tabs.history.noHistory")}
                    </div>
                  )}
                </div>
              )}

    ...
    <div className="flex items-start gap-3 p-3 rounded-lg border border-border hover:bg-muted/50 transition-colors duration-base">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h4 className={`text-sm font-medium ${task?.completed ? "line-through text-muted-foreground" : "text-foreground"}`}>{task?.title}</h4>
      ...
      <button onClick={() => onViewDetails(task?.id)} className="flex-shrink-0 p-1 hover:bg-muted rounded transition-colors duration-base focus-clinical" aria-label="View task details">

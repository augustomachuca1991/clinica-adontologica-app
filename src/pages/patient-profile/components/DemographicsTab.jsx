  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
      {demographicSections?.map((section, index) => (
        <div key={index} className="bg-card rounded-lg border border-border p-4 md:p-5 clinical-card">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0">
              <Icon name={section?.icon} size={20} color="var(--color-primary)" />
            </div>
            <h3 className="text-base md:text-lg font-headline font-semibold text-foreground">{section?.title}</h3>
          </div>

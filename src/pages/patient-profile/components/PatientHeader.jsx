...
const PatientHeader = ({ patient, onEdit, onSchedule, onMessage }) => {
  ...
  const statusColors = {
    active: { background: "bg-success", label: t("common.status.active") },
    pending: { background: "bg-warning", label: t("common.status.pending") },
    inactive: { background: "bg-muted-foreground", label: t("common.status.inactive") },
  };
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
        <div className="flex-shrink-0">
          <div className="relative w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40">
            <Image
              src={patient?.avatar}
              alt={patient?.avatar_alt}
              className="w-full h-full rounded-lg object-cover border-2 border-primary/20"
            />
            <div
              className={`absolute bottom-2 right-2 w-4 h-4 md:w-5 md:h-5 rounded-full border-2 border-card ${statusColors[patient?.status].background}`}
            />
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 md:gap-4 mb-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 md:gap-3 mb-2">
                ...
                <span
                  className={`status-indicator ${getStatusColor(patient?.status)} text-xs md:text-sm px-2 md:px-3 py-1 rounded-full flex-shrink-0`}
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground">{t("common.labels.gender")}</p>
                    <p className="text-sm md:text-base font-medium text-foreground truncate">
                      {patient?.gender ? t(`gender.${patient?.gender}`) : "N/A"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">

...
const SettingsSection = ({ title, description, icon, children, collapsible = false, defaultOpen = true }) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-clinical-sm overflow-hidden">
      <div className={`px-6 py-4 border-b border-border bg-muted/30 ${collapsible ? "cursor-pointer hover:bg-muted/50" : ""}`} onClick={collapsible ? () => setIsOpen(!isOpen) : undefined}>
        <div className="flex items-center justify-between">
          ...
          {collapsible && <Icon name={isOpen ? "ChevronUp" : "ChevronDown"} size={20} className="text-muted-foreground transition-transform duration-base" />}
        </div>
      </div>
